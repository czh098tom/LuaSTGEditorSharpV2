...
{
    [NodeCreationMenu("Operator/Math", EnglishTitle = "Sqrt", Order = 12)]
    public class SqrtNode : LinqSTGNodeViewModel
    {
        ...
        public SqrtNode()
        {
            ...
            OutputValue.Value = InputX.ValueChanged
                .Select(x => Contextual.Create(dict => MathF.Sqrt(x?.Invoke(dict) ?? 0f)));
        }
    }
}
