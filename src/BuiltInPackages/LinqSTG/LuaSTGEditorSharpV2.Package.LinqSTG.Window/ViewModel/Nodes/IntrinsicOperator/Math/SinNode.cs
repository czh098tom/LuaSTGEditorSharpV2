...
{
    [NodeCreationMenu("Operator/Math", EnglishTitle = "Sin", Order = 0)]
    public class SinNode : LinqSTGNodeViewModel
    {
        ...
        public SinNode()
        {
            ...
            OutputValue.Value = InputX.ValueChanged
                .Select(x => Contextual.Create(dict => MathF.Sin((x?.Invoke(dict) ?? 0f) * MathF.PI / 180f)));
        }
    }
}
