...
{
    [NodeCreationMenu("Operator/Math", EnglishTitle = "Exp", Order = 13)]
    public class ExpNode : LinqSTGNodeViewModel
    {
        ...
        public ExpNode()
        {
            ...
            OutputValue.Value = InputX.ValueChanged
                .Select(x => Contextual.Create(dict => MathF.Exp(x?.Invoke(dict) ?? 0f)));
        }
    }
}
