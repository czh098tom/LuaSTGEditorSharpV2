...
{
    [NodeCreationMenu("Operator/Math", EnglishTitle = "Ceil")]
    public class CeilNode : LinqSTGNodeViewModel
    {
        ...
        public CeilNode()
        {
            ...
            OutputValue.Value = InputX.ValueChanged
                .Select(x => Contextual.Create(dict => MathF.Ceiling(x?.Invoke(dict) ?? 0f)));
        }
    }
}
