...
{
    [NodeCreationMenu("Operator/Math", EnglishTitle = "ATan")]
    public class ATanNode : LinqSTGNodeViewModel
    {
        ...
        public ATanNode()
        {
            ...
            OutputValue.Value = InputX.ValueChanged
                .Select(x => Contextual.Create(dict => MathF.Atan(x?.Invoke(dict) ?? 0f) * 180f / MathF.PI));
        }
    }
}
