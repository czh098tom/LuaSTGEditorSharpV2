...
{
    [NodeCreationMenu("Operator", EnglishTitle = "Divide (/)", Order = 3)]
    public class DivideNode : LinqSTGNodeViewModel
    {
        ...
        public DivideNode()
        {
            ...
            NumericResult.Value = NumericA.ValueChanged.CombineLatest(NumericB.ValueChanged, TryDivide);
        }

        private static object? TryDivide(object? lhs, object? rhs)
        {
            ...
            if (lhs is Contextual<Vector2> cvec1 && rhs is Contextual<Vector2> cvec2)
            {
                return Contextual.Create(dict =>
                    new Vector2(cvec1(dict).X / cvec2(dict).X, cvec1(dict).Y / cvec2(dict).Y));
            }
            return null;
        }
    }
