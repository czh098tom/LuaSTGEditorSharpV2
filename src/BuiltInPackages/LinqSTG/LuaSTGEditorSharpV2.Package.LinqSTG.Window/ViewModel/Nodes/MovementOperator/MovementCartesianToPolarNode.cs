...
{
    /// <summary>
    /// 把上游运动预测点当作 (角度θ=度, 半径r) 解释为极坐标，展开回笛卡尔坐标。
    ...
    /// </summary>
    [NodeCreationMenu("Movement/Operator", TitleKey = "linqstg_window_node_movementCartesianToPolar", Order = 9)]
                ...
                .CombineLatest(InputCenter.ValueChanged, (movement, center)
                    => Contextual.Create(dict
                        => new Parametric<float, Vector2>(t =>
                        {
                            ...
                            var q = p - c;
                            var rad = q.X * MathF.PI / 180f;
                            var cos = MathF.Cos(rad);
                            var sin = MathF.Sin(rad);
                            return c + new Vector2(q.Y * cos, q.Y * sin);
                        })));
        }
    }
}
